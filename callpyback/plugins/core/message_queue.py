...
class MessageQueue:
            ...
            for topic, subs in self.subscriptions.items():
                for i, sub in enumerate(subs):
                    if sub.id == subscription_id:
                        subs.pop(i)
                        self.stats['subscriptions_active'] -= 1
                        logger.debug(f"Unsubscribed {subscription_id} from topic '{topic}'")
                        return True
        return False
    ...
    def _message_matches_filters(self, message: Message, filters: Dict[str, Any]) -> bool:
    ...
    def _handle_message_error(self, message: Message, subscription: Subscription, error: Exception):
        ...
        if subscription.retry_count <= subscription.max_retries:
            ...
            logger.debug(f"Retrying message {message.id} (attempt {subscription.retry_count})")
        else:
            ...
            logger.warning(f"Message {message.id} sent to dead letter queue after {subscription.retry_count} failures")
    
    def get_stats(self) -> Dict[str, Any]:
        """Get queue statistics."""
        with self.lock:
            queue_lengths = {topic: len(msgs) for topic, msgs in self.message_queues.items()}
