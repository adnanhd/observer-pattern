...
if HAS_FASTAPI:
    ...
    class TaskSubmissionRequest(BaseModel):
        ...
        strategy: Optional[str] = Field(
            None, description="Execution strategy (thread/process)"
        )
...
class RESTAPIPlugin:
    ...
    def __init__(
        ...
        if not HAS_FASTAPI:
            raise ImportError(
                "FastAPI is required for REST API plugin. Install with: pip install fastapi uvicorn"
            )
    ...
    def _setup_routes(self):
            ...
            try:
                ...
                async def message_handler(message: Message):
                    ...
                    if request.webhook_url:
                        await self.webhook_handler.notify_webhook(
                ...
                if request.webhook_url:
                    await self.webhook_handler.add_webhook(
                        subscription_id, request.webhook_url
                    )
        ...
        async def submit_task(request: TaskSubmissionRequest):
            ...
            if not self.executor:
                raise HTTPException(
                    status_code=503, detail="Task executor not available"
                )
            ...
            try:
                ...
                if hasattr(self.executor, "submit"):
                    ...
                    raise HTTPException(
                        status_code=503,
                        detail="Executor does not support task submission",
                    )
        ...
        if self.enable_websockets:
            ...
            async def websocket_endpoint(websocket, topic: str):
                ...
                try:
                    ...
                    # Clean up connection
                    if (
                        topic in self.websocket_connections
                        and connection_id in self.websocket_connections[topic]
                    ):
    ...
    def start(self, background: bool = True):
        ...
        if background:
            ...
            self.server_thread = threading.Thread(
                target=self._run_server, daemon=True, name="RESTAPIServer"
            )
