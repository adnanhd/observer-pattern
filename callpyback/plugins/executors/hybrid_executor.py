...
class ExecutionStrategy:
    ...
    cpu_bound_keywords: List[str] = field(default_factory=lambda: [
        'compute', 'calculate', 'process', 'algorithm', 'math', 'crypto',
        'encode', 'decode', 'compress', 'parse', 'transform'
    ])
    
    # I/O-bound indicators  
    io_bound_keywords: List[str] = field(default_factory=lambda: [
        'fetch', 'download', 'upload', 'request', 'query', 'save',
        'load', 'read', 'write', 'api', 'database', 'file'
    ])
...
class TaskClassifier:
    ...
    def classify_task(self, 
        ...
        # CPU-bound indicators
        if self._contains_keywords(func_name + ' ' + func_doc, self.strategy.cpu_bound_keywords):
            return 'process'
        
        # I/O-bound indicators
        if self._contains_keywords(func_name + ' ' + func_doc, self.strategy.io_bound_keywords):
            return 'thread'
...
class HybridExecutor:
    ...
    def get_result(self, task_id: str, timeout: Optional[float] = None) -> Union[TaskResult, ProcessResult]:
    ...
    def wait_for_completion(self,
                          ...
                          timeout: Optional[float] = None) -> Dict[str, Union[TaskResult, ProcessResult]]:
    ...
    def optimize_strategy(self) -> Dict[str, Any]:
        ...
        for func_name, data in history.items():
            ...
            if avg_cpu_time > self.strategy.cpu_bound_threshold and thread_count > process_count:
            ...
            if avg_memory > self.strategy.memory_threshold and thread_count > process_count:
                recommendations.append({
                    'function': func_name,
                    'current': 'mostly_thread', 
                    'recommended': 'process',
        ...
        return {
            ...
            'strategy_distribution': {
                'thread_preference': sum(1 for d in history.values() if d.get('thread_count', 0) > d.get('process_count', 0)),
                'process_preference': sum(1 for d in history.values() if d.get('process_count', 0) > d.get('thread_count', 0))
            }
        }
